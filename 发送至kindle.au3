#cs -------------------------------------------------------------------------
        ...
        Last modified: jiangming

        Script Function:
        Monitors the user defined directories for book file activity and sendfile to kindle.
        ...
        While $nNext <> 0
                ...
                Switch DllStructGetData($hFileNameInfo, "Action")
                        Case $FILE_ACTION_ADDED
								...
								$filename=DllStructGetData($hFileName, "FileName")
								if StringInStr($filename,".mobi") then
									$rc = _INetSmtpMailCom($s_SmtpServer, $s_FromName, $s_FromAddress, $s_ToAddress, $filename, $as_Body, $sDir &"\"& DllStructGetData($hFileName, "FileName"), $s_CcAddress, $s_BccAddress, $s_Username, $s_Password, $IPPort, $ssl)
								 endif
								if StringInStr($filename,".pdf") then
									$rc = _INetSmtpMailCom($s_SmtpServer, $s_FromName, $s_FromAddress, $s_ToAddress, $filename, $as_Body, $sDir &"\"& DllStructGetData($hFileName, "FileName"), $s_CcAddress, $s_BccAddress, $s_Username, $s_Password, $IPPort, $ssl)
								 endif
								if StringInStr($filename,".epub") then
									$rc = _INetSmtpMailCom($s_SmtpServer, $s_FromName, $s_FromAddress, $s_ToAddress, $filename, $as_Body, $sDir &"\"& DllStructGetData($hFileName, "FileName"), $s_CcAddress, $s_BccAddress, $s_Username, $s_Password, $IPPort, $ssl)
								 endif
								if StringInStr($filename,".txt") then
									$rc = _INetSmtpMailCom($s_SmtpServer, $s_FromName, $s_FromAddress, $s_ToAddress, $filename, $as_Body, $sDir &"\"& DllStructGetData($hFileName, "FileName"), $s_CcAddress, $s_BccAddress, $s_Username, $s_Password, $IPPort, $ssl)
								 endif
